...
export const createApplication = async (
  ...
  try {
    ...
    const validatedStatus = status as ApplicationStatus;
    ...
    res
      .status(500)
      .json({ message: `Error creating application: ${message}` });
  }
};
...
export const updateApplicationStatus = async (
  ...
  try {
    ...
    if (validatedStatus === ApplicationStatus.Approved) {
      ...
      await prisma.application.update({
        where: { id: Number(id) },
        data: { status: validatedStatus, leaseId: newLease.id },
        include: {
          property: true,
          tenant: true,
          lease: true,
    ...
    res
      .status(500)
      .json({ message: `Error updating application status: ${message}` });
  }
};